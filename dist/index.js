...
class Node {
  ...
  checkReadAccess () {
    return checkAccess(
      this.perms,
      fs$1.constants.S_IRUSR,
      fs$1.constants.S_IRGRP,
      fs$1.constants.S_IROTH,
      this.uid,
      this.gid
    )
  }
  checkWriteAccess () {
    return checkAccess(
      this.perms,
      fs$1.constants.S_IWUSR,
      fs$1.constants.S_IWGRP,
      fs$1.constants.S_IWOTH,
      this.uid,
      this.gid
    )
  }
  checkExecuteAccess () {
    return checkAccess(
      this.perms,
      fs$1.constants.S_IXUSR,
      fs$1.constants.S_IXGRP,
      fs$1.constants.S_IXOTH,
      this.uid,
      this.gid
    )
  }
  ...
  access (mode) {
    ...
    if ((mode & fs$1.constants.R_OK) !== 0 && !this.checkReadAccess()) return false
    if ((mode & fs$1.constants.W_OK) !== 0 && !this.checkWriteAccess()) return false
    if ((mode & fs$1.constants.X_OK) !== 0 && !this.checkExecuteAccess()) return false
    return true
  }
}
...
class Symlink extends Node {
  constructor (target, mode = DEFAULT_FILE_MODE) {
    ...
    super((mode & 0o777) | fs$1.constants.S_IFLNK);
...
class File extends Node {
  constructor (mode = DEFAULT_FILE_MODE$1) {
    ow(mode, ow.number.integer);
    super((mode & 0o777) | fs$1.constants.S_IFREG);
  ...
  open (flags) {
    ...
    if ((flags & 3) === fs$1.constants.O_RDONLY) {
      this.ensureReadAccess();
    } else if ((flags & 3) === fs$1.constants.O_WRONLY) {
...
class Filehandle {
  constructor (file, flags) {
    ...
    if ((this._flags & fs$1.constants.O_TRUNC) !== 0) this.file.truncate(0);
    this._pos = (this._flags & fs$1.constants.O_APPEND) !== 0 ? this.file.size : 0;
  }
  close () {
    fds.release(this.fd);
  ...
  _ensureReadble () {
    if ((this._flags & 3) === fs$1.constants.O_WRONLY) throw makeError('EACCES')
  }
  _ensureWritable () {
    if ((this._flags & 3) === fs$1.constants.O_RDONLY) throw makeError('EACCES')
  }
  write (buffer, ...args) {
...
class Directory extends Node {
  constructor (mode = DEFAULT_DIR_MODE) {
    ow(mode, ow.number);
    super((mode & 0o777) | fs$1.constants.S_IFDIR);
    this._links = new Map();
  }
  get (name) {
    ow(name, ow.string);
  ...
  find (path$1, { toDir = false, followLinks = true } = {}, limit = 100) {
    let node = this;
    let step;
    let stepsLeft = path.resolve(path$1)
      .slice(1)
      .split('/')
      .filter(Boolean);
    let stepsTaken = [];
    ...
    path$1 = '/' + stepsTaken.join('/');
    if (toDir) {
      return { path: path$1, node, name: stepsLeft[0] }
    } else {
...
class Dirent extends fs$1.Dirent {
  constructor (name, stats) {
    super(name, null);
    this[kStats] = stats;
  }
}
for (const name of Reflect.ownKeys(fs$1.Dirent.prototype)) {
...
class Filesystem {
  ...
  mkdir (path$1, options = {}) {
    ow(path$1, 'path', ow.string.nonEmpty);
    ow(options, 'modeOrOptions', ow.any(ow.number.integer, ow.object));
    if (typeof options === 'number') options = { mode: options };
    const { recursive = false, mode } = options;
    try {
      const { node: dir, name } = this.root.findDir(path$1);
      dir.mkdir(name, mode);
    } catch (err) {
      if (err.code === 'ENOENT' && recursive) {
        this.mkdir(path.dirname(path$1), options);
        return this.mkdir(path$1, options)
      }
      throw err
    }
  }
  ...
  access (path, mode = fs$1.constants.F_OK) {
    ow(path, 'path', ow.string.nonEmpty);
    try {
  ...
  open (path, flags, mode) {
    ...
    if ((flags & fs$1.constants.O_CREAT) === 0) {
      const { node: file } = this.root.find(path);
      if (!(file instanceof File)) throw makeError('EISDIR')
    ...
    if (!(file instanceof File)) throw makeError('EISDIR')
    if ((flags & fs$1.constants.O_EXCL) !== 0) throw makeError('EEXIST')
    return file.open(flags)
  }
  ...
  copyFile (sourcePath, destinationPath, flags = 0) {
    ...
    ow(flags, ow.number.integer);
    const flag = flags & fs$1.constants.COPYFILE_EXCL ? 'wx' : 'w';
...
const OPEN_MODES = {
  ...
  'w+': fs$1.constants.O_RDWR | fs$1.constants.O_CREAT | fs$1.constants.O_TRUNC,
  'wx+': fs$1.constants.O_RDWR | fs$1.constants.O_CREAT | fs$1.constants.O_TRUNC | fs$1.constants.O_EXCL,
  a: fs$1.constants.O_WRONLY | fs$1.constants.O_APPEND | fs$1.constants.O_CREAT,
  ax: fs$1.constants.O_WRONLY | fs$1.constants.O_APPEND | fs$1.constants.O_CREAT | fs$1.constants.O_EXCL,
  'a+': fs$1.constants.O_RDWR | fs$1.constants.O_APPEND | fs$1.constants.O_CREAT,
  'ax+': fs$1.constants.O_RDWR | fs$1.constants.O_APPEND | fs$1.constants.O_CREAT | fs$1.constants.O_EXCL
};

class MemFS {
  constructor () {
    const fs = new Filesystem();
    this.constants = fs$1.constants;
    this.Stats = fs$1.Stats;
    this.ReadStream = fs$1.ReadStream;
    this.WriteStream = fs$1.WriteStream;
